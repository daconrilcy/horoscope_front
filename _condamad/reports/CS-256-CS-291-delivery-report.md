...
Update-time evidence from CS-292, CS-293 and CS-294 changes that closure view: CS-284 is now `Delivered`, CS-268 is closed for the current runtime surface, and CS-262 is now administratively `done` as an audit/reconciliation story. Latest update: CS-301 revalidates the CS-300 payload/hash repair and closes CS-278 as `done` for the approved `replay_snapshot_v1` runtime.
...
| CS-278 | Implement `replay_snapshot_v1` after approval. | `_condamad/stories/CS-278-replay-snapshot-v1-implementation/00-story.md` | Runtime closed by CS-295 through CS-301 after CS-300 repair; internal admin exposure only. |
...
- CS-278 originally stopped before implementation because `approval_state` was `non approuve`; latest update closes the approved runtime through CS-295, CS-296, CS-297, CS-298, CS-300 and CS-301.
...
| CS-278 | Replay snapshot runtime after approval. | `_story_briefs/cs-278-*.md` | Runtime implementation closed by CS-295 through CS-301 after CS-300 payload/hash repair. | DPO/security approval request is `approved`; CS-301 records real `log_call -> snapshot -> replay` proof, full backend lint, full pytest, OpenAPI/routes/TestClient and forbidden-data scan evidence. | Delivered |
...
- CS-278 originally deviated by stopping before implementation because CS-277 approval remained `non approuve` and `DPO-REPLAY-SNAPSHOT-V1-RETENTION-001` was open. Update: approval is recorded and the approved runtime is closed by CS-295 through CS-301 with local backend validation after the CS-300 payload/hash repair.
...
- Story registry `done` means the story owner intended delivery closure unless contradicted by final evidence. CS-278 is `done` after CS-301 revalidated the CS-300 repair and the CS-295 through CS-298 runtime evidence.
