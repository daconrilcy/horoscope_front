...
La validation CS-340 confirme que la correction CS-339 est effective: le prompt natal moderne derive le payload provider depuis les blocs `prompt_visible` canoniques et ne transmet plus le bloc `provenance` complet au LLM. Le statut CS-339 est `done` dans `_condamad/stories/story-status.md`.

Le gateway conserve l'objet complet `llm_astrology_input_v1` comme source auditable, mais compose le message provider avec une projection prompt-visible. Les champs de provenance, hash et persistance restent disponibles pour l'audit narratif et la persistance.
...
Le gateway lit `LLM_ASTROLOGY_INPUT_DATA_ROLES["prompt_visible"]` via `LLM_ASTROLOGY_INPUT_V1_PROMPT_BLOCKS` et applique `_prompt_visible_llm_astrology_input(...)` avant la composition du message utilisateur.
...
- `backend/app/domain/llm/runtime/gateway.py`: projection provider `_prompt_visible_llm_astrology_input`.
...
- `backend/tests/llm_orchestration/test_llm_astrology_input_boundaries.py`: payload final provider et message utilisateur inspectes avec double local.
- `backend/tests/architecture/test_llm_astrology_input_payload_boundaries.py`: guard AST contre divergence des roles et champs audit-only.
...
- guard test: `test_llm_astrology_input_boundaries.py` et `test_llm_astrology_input_payload_boundaries.py`, assertions negatives sur `provenance`, `projection_hash`, `llm_input_hash`, `provider_response`, `persisted_answer`, `chart_json`, `natal_data`.
...
- `ruff check .`: PASS.
- `python -B -m pytest -q tests/unit/domain/astrology/test_llm_astrology_input_v1.py tests/unit/domain/astrology/test_llm_astrology_input_hash.py tests/unit/domain/astrology/test_llm_astrology_input_evidence.py tests/llm_orchestration/test_llm_astrology_input_boundaries.py tests/architecture/test_llm_astrology_input_payload_boundaries.py tests/integration/test_llm_legacy_extinction.py tests/integration/llm/test_natal_llm_astrology_input_audit.py --tb=short`: PASS, 23 passed, 9 deselected.
- `python -B -m pytest -q tests --tb=short`: PASS, 1210 passed, 221 deselected.
- `rg -n "provenance|projection_hash|llm_input_hash|audit_only|prompt_visible" app tests ..\_condamad ..\_story_briefs`: PASS, resultats sauvegardes.
- `rg -n "{{provenance}}|{{projection_hash}}|{{llm_input_hash}}" app tests`: PASS, aucune occurrence.

## Risques residuels

- Les references historiques dans `_condamad/**` et `_story_briefs/**` restent nombreuses; elles sont classees comme evidence historique et ne sont pas des surfaces executables.
- Le rapport ne modifie pas le code runtime parce que les tests et guards existants prouvent deja la correction CS-339.
- Aucun appel provider reel n'a ete effectue; la preuve de handoff repose sur le double local inspectant le message juste avant la frontiere provider.
