...
| AC2 | Sensitive admin actions name audit events. | The contract maps reads, review updates, replay metadata reads, replay attempts and purges to existing audit events. | `evidence/validation.txt` with `python -B -m pytest -q tests/api/admin/test_rejected_answer_review_workflow.py --tb=short` PASS. | PASS |
