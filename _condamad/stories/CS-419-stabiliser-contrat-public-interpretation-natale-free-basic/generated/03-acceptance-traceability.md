...
|---|---|---|---|---|
| AC1 | Free short returns `meta.level=short`. | `NatalInterpretationService` coerces free-short public meta to `short`, including stabilized public use-case rows still stored as `COMPLETE`. | Public contract tests PASS. | PASS |
...
| AC10 | Accepted public payloads exclude technical markers. | Public serializer still removes evidence; tests assert denylisted markers, raw carriers and audit markers absent from accepted public JSON. | Public contract test PASS; targeted scans recorded in `evidence/validation.txt`. | PASS |
