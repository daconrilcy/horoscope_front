...
| AC9 | Public DOM hides technical markers. | Modern narrative branch avoids legacy markers and raw evidence IDs; obsolete complete legacy bodies show only the regeneration message. | `natalPublicDomGuard` PASS; forbidden `rg` scans for `ni-evidence-tags`, `ni-projections`, `LockedSection`, `NatalInterpretationLegacyBody`, `style=` PASS with no matches in scoped files. | PASS |
