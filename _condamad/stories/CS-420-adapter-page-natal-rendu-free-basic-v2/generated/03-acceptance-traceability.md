...
|---|---|---|---|---|
| AC1 | Free short renders its primary public reading. | `NatalInterpretationContent.tsx` renders free public title and summary for `natal_long_free`; `natalInterpretation.test.tsx` covers the complete free_short branch. | `pnpm --dir frontend test -- natalInterpretation natalPublicDomGuard natalNarrativeReading NatalChartPage component-architecture-guards component-usage-guards`; full `pnpm --dir frontend test`. | PASS |
| AC2 | Free short renders its supporting public blocks. | Free public sections, highlights, advice and payload disclaimers render with `ni-public-*` classes in `NatalInterpretationContent.tsx` and CSS in `NatalInterpretation.css`. | Same targeted Vitest command; `NatalChartPage.test.tsx` updated to expect public sections without legacy accordion classes. | PASS |
...
| AC6 | Basic V2 evidence renders only public labels. | `PublicEvidenceList` renders only `label` and `meaning`; no theme/raw ids are displayed. | `natalPublicDomGuard.test.tsx`; scan for forbidden public technical markers. | PASS |
