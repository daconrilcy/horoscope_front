...
| AC7 | Downstream Basic uses eligibility. | `llm_astrology_input_v1.py` imports and calls canonical eligibility filters for facts, signals and shaping. | AST guard and runtime `shaping.support_elements` guard PASS. | PASS |
| AC8 | Non-time-dependent facts remain available. | Date-only LLM filtering keeps zodiac positions, sign balances and major aspects while clearing house data and blocked shaping appuis. | `test_date_only_llm_input_removes_house_surfaces_but_keeps_core_facts` PASS. | PASS |
