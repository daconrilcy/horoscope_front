...
| AC7 | Unsupported generated claims fail validation. | `unsupported_claims` in generated payload produce a rejected outcome with `natal_output_policy_violation`. | `pytest ... test_rejected_narrative_answer_workflow.py::test_unsupported_generated_claim_becomes_rejected` PASS. | PASS |
| AC8 | Ignored critical limits fail validation. | `ignored_critical_limits` in generated payload produce a rejected outcome with `natal_output_policy_violation`. | `pytest ... test_rejected_narrative_answer_workflow.py::test_ignored_critical_limit_becomes_rejected` PASS. | PASS |
