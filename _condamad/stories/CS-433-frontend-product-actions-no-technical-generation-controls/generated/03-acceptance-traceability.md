...
| AC10 | The UI renders controlled slot states. | API hook exposes normalized `ThemeNatalReadingSlotState`; `NatalInterpretation` handles generating, failed, locked, paywall, and rejected branches. | `pnpm --dir frontend test -- natalInterpretation NatalChartPage natalChartApi component-architecture` PASS with explicit slot-state cases. | PASS |
