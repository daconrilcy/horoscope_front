...
| AC14 | Forbidden formulations are guarded. | Every `ThemeDefinition` declares `forbidden_formulations`; app scan uses standalone forbidden wording boundaries. | `rg -n "\b(spirituel\|creatif\|harmonieux\|profond)\b" ...` returned no matches; literal scan has pre-existing `approfondie/profondeur` substring false positives recorded in `evidence/validation.txt`. | PASS_WITH_LIMITATIONS |
