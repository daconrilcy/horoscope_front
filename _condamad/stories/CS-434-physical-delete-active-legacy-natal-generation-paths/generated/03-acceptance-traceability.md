...
|---|---|---|---|---|
| AC1 | Public generation cannot use `natal_interpretation_short`. | `users.py` closes remaining public generation entrypoints; `adapter.py` rejects deleted key; prompt catalog and bootstrap no longer publish it. | `app.routes`/`app.openapi()` checks PASS; architecture guard PASS; targeted `rg` allowlist only. | PASS |
| AC2 | Public generation cannot use `natal_long_free`. | `PROMPT_RUNTIME_DATA` no longer contains `natal_long_free`; adapter rejects it; old nominal tests converted. | Catalog runtime check PASS; `llm_orchestration` full PASS; scan after captured. | PASS |
