...
| `natal_interpretation_short`, `natal_long_free` | `backend/tests/**` | Assertions anti-retour, fixtures historiques ou tests readonly. | test-guard | Tests nominaux convertis dans `test_context_quality.py`, `test_resolved_execution_plan.py`, `test_runtime_convergence.py`, `test_ai_engine_adapter.py`, `test_natal_interpretation_service_v2.py`. | backend-tests |
