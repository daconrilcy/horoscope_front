...
| Projection scan | PASS | `json_builder.py` contains only `_serialize_planet_dominance` and payload projection; no `PlanetDominanceEngine` call. |
...
| Chart balance classification | PASS | `chart_balance.dominant_planets` remains the pre-existing structural balance field sourced from `sign_runtime`; `planet_dominance` is the canonical factual planetary dominance output for CS-194. |
...
`planet_dominance` schema. No route, HTTP method, status code or declared
