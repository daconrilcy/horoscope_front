...
| AC5 | Chaque score contient un breakdown par facteur. | `PlanetDominancePlanetResult.factors`. | `pytest -q backend/tests/unit/domain/astrology/test_planet_dominance_engine.py` -> PASS. | PASS |
...
| AC9 | `NatalResult` expose `planet_dominance` sans modifier `chart_balance.dominant_planets`. | Champ `NatalResult.planet_dominance`, calcul apres `chart_balance`. | `pytest -q backend/tests/unit/domain/astrology/test_natal_result_contract.py` -> PASS. | PASS |
| AC10 | `build_chart_json` expose `planet_dominance` sans recalcul. | `_serialize_planet_dominance` projette `NatalResult.planet_dominance`. | `pytest -q backend/app/tests/unit/test_chart_json_builder.py backend/app/tests/unit/test_chart_result_service.py` -> PASS. | PASS |
