...
| AC2 | No positive `natal_interpretation_short` generation test remains. | Positive mock scans no matches; architecture guard suite `54 passed`. | PASS | No hit remains in `interpretation_service.py`. |
| AC3 | No positive `natal_long_free` generation test remains. | Backend theme natal suite `50 passed`; architecture guard suite `54 passed`. | PASS | No hit remains in `interpretation_service.py`. |
...
- Updated public free/basic contract and architecture guard tests during CS-444 final alignment.
...
| Backend theme natal product/read pytest suite | `backend` | PASS | 0 | `50 passed` with `--long`. |
...
- Old free-preview use-case keys were removed from public formatting in `interpretation_service.py`.
...
- Residual old-key literals are not nominal public generation: they are readonly historical outside public generator code, admin-only, rejection guard,
  or extinction/proof-test evidence.
