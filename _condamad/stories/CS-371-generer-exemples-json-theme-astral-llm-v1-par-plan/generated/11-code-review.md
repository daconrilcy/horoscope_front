...
- Fixed in this cycle: implementation/brief alignment. The generator previously built planet, house, and aspect
  `InterpretationMaterialSource` entries directly in the story evidence script. It now loads those sources through
  `InterpretationMaterialSourceRepository` from seeded SQLite interpretation-profile tables, then keeps only the runtime supplemental
  sections locally.
- Fixed in this cycle: stale review evidence. The previous `generated/11-code-review.md` did not include the post-fix source-table
  alignment check.
...
- PASS: `interpretation_material` includes DB-profile source refs from `astral_planet_interpretation_profiles`,
  `astral_house_interpretation_profiles`, and `astral_aspect_interpretation_profiles`.
