...
|---|---|---|---|---|
| AC1 | Three advanced condition reference tables exist and are seeded. | Migration, SQLAlchemy models, JSON seed sync. | Runtime repository and migration tests passed. | PASS |
...
| AC7 | Solar phase/orientation V1 is detected. | `heliacal_condition_calculator.py`, real accidental dignity source evaluation. | `test_heliacal_conditions.py`, `test_accidental_dignity_calculator.py` and natal pipeline test passed. | PASS |
| AC8 | Aspect conditions V1 are detected. | `aspect_condition_detector.py` with condition-compatible target selection. | `test_besiegement_detector.py` and natal pipeline test passed. | PASS |
