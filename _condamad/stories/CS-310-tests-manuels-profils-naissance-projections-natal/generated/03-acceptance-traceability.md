...
| AC2 | Every profile has a traced `/natal` result. | `evidence/manual-qa-ledger.json` contains one entry per profile with `visible_result`, `execution_mode`, and `evidence_path`. | Python evidence assertion checks ledger/profile ID equality and route `/natal`. | PASS |
