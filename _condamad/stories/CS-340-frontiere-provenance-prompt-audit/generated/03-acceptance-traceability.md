...
| AC3 | Provider handoff payload excludes audit-only fields. | `test_gateway_provider_handoff_uses_local_double_and_prompt_boundary` inspects `mock_client.execute` messages just before provider boundary. | `python -B -m pytest -q tests/llm_orchestration/test_llm_astrology_input_boundaries.py ...` PASS. | PASS |
...
| AC6 | Backend validation commands pass. | No runtime code change was required; existing guards prove the corrected boundary. | `ruff format --check app tests`, `ruff check .`, targeted pytest, and full `python -B -m pytest -q tests --tb=short` PASS. | PASS |
