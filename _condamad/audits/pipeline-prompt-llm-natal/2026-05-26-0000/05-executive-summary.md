...
The current natal LLM pipeline carries rich runtime context into `LLMGateway`, but only `chart_json` is prompt-visible by default. If the developer prompt assembled through `assembly_resolver` and rendered through `PromptRenderer` contains `{{chart_json}}`, that placeholder owns injection; otherwise the gateway appends `Technical Data: {chart_json}` to the user data block.
