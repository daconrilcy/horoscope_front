...
The post-refactor frontend design-system state is green and better than the `2026-05-05-2053` baseline. Targeted design-system guards, visual smoke, the full Vitest suite, lint, and production build pass.

The remaining debt is governed, not blocking: 24 CSS fallback exceptions across 10 CSS files, 14 inline style exceptions across 9 TSX files, broad hardcoded visual or typography signals across 116 files, and 17 classified legacy selector/token rows. No Critical or High finding was found.
...
- Impact: the previous frontend design-system refactors are protected by executable checks.
...
- Actual state: `CSS_FALLBACK_EXCEPTIONS` contains 24 exact entries. The scan finds 23 source lines across 10 CSS files; the count differs because a line can contain nested fallback expressions. The largest remaining clusters are `PeriodCard.css`, `NatalInterpretation.css`, `KeyPointCard.css`, and `NatalChartPage.css`.
- Impact: fallback literals still preserve alternate visual decisions beside the canonical token set.
- Recommended action: reduce fallbacks in bounded batches and update `css-fallback-allowlist.md` plus `design-system-allowlist.ts` in the same change.
- Story candidate: yes
...
- Actual state: 14 inline style attributes remain across 9 TSX files and are covered by exact allowlists.
- Impact: justified dynamic values still keep styling decisions in TSX, so the allowlist must not grow silently.
- Recommended action: convert removable color/background/position bridges to CSS custom properties or classes where practical; keep runtime geometry and explicit style-prop pass-throughs.
- Story candidate: yes
- Exhaustive files to modify:
...
- Actual state: the broad source scan finds 116 files with hardcoded visual or typography signals outside canonical token source files.
...
- Actual state: `legacy-style-surface-registry.md` contains 17 active classified rows. Migration-only chat selector families still live in `App.css`, external-active admin prompt selectors still live in `AdminPromptsPage.css`, and compatibility token aliases remain in `theme.css`.
...
Start with `F-002`, targeting `PeriodCard.css`, `NatalInterpretation.css`, `KeyPointCard.css`, and `NatalChartPage.css` as a bounded fallback-reduction batch. Then handle `F-003` only where a CSS custom-property bridge can reduce TSX styling without losing runtime behavior.
