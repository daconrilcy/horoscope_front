...
53. **AC53 - Suppression complete des champs legacy d assembly** : les champs `execution_config`, `interaction_mode`, `user_question_policy`, `input_schema`, `output_contract_ref` et `fallback_use_case` ne doivent plus exister dans `PromptAssemblyConfigModel`, ni comme colonnes SQLAlchemy, ni comme proprietes, ni comme alias d initialisation.
54. **AC54 - Migration destructive controlee des colonnes legacy d assembly** : une migration Alembic doit supprimer physiquement les colonnes legacy d assembly de la table `llm_assembly_configs`, apres verification que leur contenu est soit migre vers les sources canoniques (`execution_profile_ref`, `output_schema_id`, release snapshot, admin read model), soit explicitement abandonne comme historique non nominal.
55. **AC55 - Suppression complete du registre de compatibilite assembly** : `ASSEMBLY_COMPATIBILITY_SPECS` ne doit plus contenir aucun champ d assembly. Si le fichier `llm_compatibility.py` ne contient plus que des residus supprimes, il doit etre supprime entierement ou reduit uniquement aux compatibilites encore reellement necessaires hors assembly.
56. **AC56 - Suppression complete des alias legacy d assembly** : tout acces applicatif, admin, script, seed, migration ou test a `output_contract_ref`, `execution_config`, `fallback_use_case`, `interaction_mode`, `user_question_policy` ou `input_schema` doit etre remplace par les champs ou services canoniques. Aucun `getattr`, fallback silencieux, mapping, serializer ou DTO ne doit conserver ces noms legacy.
57. **AC57 - Runtime nominal strictement profile-owned** : la resolution runtime ne doit lire les parametres d execution que depuis `LlmExecutionProfileModel`, le snapshot actif ou les objets canoniques explicitement definis. Aucun parametre runtime (`provider`, `model`, `temperature`, `timeout_seconds`, `max_output_tokens`, `reasoning_effort`, `verbosity`) ne doit pouvoir etre lu depuis une configuration d assembly ou depuis une version de prompt.
58. **AC58 - Suppression complete des champs execution legacy dans `LlmPromptVersionModel`** : les champs `model`, `temperature`, `max_output_tokens`, `reasoning_effort` et `verbosity` ne doivent plus etre consideres comme porteurs d execution. Ils doivent etre supprimes du modele et de la table si plus aucun flux historique ne les consomme, ou migres vers `LlmExecutionProfileModel` avant suppression.
59. **AC59 - Migration destructive controlee des champs execution legacy des prompt versions** : une migration Alembic doit supprimer physiquement les colonnes d execution legacy de `llm_prompt_versions` apres migration ou abandon documente des valeurs historiques. Les tests doivent verifier que `developer_prompt` reste la seule source de verite texte du prompt.
60. **AC60 - Suppression des compatibilites legacy de `LlmUseCaseConfigModel`** : les champs et marqueurs legacy autour de `fallback_use_case_key`, `allowed_persona_ids`, `interaction_mode`, `user_question_policy`, `persona_strategy`, `safety_profile`, `input_schema` et `output_schema_id` doivent etre supprimes ou deplaces vers un read model non runtime si un besoin admin historique demeure. Le modele `LlmUseCaseConfigModel` ne doit plus etre presente comme une source de verite runtime.
61. **AC61 - Clarification ou retrait complet du modele `LlmUseCaseConfigModel`** : soit `LlmUseCaseConfigModel` est supprime si le paradigme `use_case` n a plus de role canonique, soit il est renomme ou isole comme modele historique/admin non runtime, avec un garde-fou empechant tout usage dans la resolution LLM nominale.
62. **AC62 - Suppression de `provider_compat` dans les logs LLM** : le champ `provider_compat` doit etre supprime de `LlmCallLogModel` et de la table `llm_call_logs` des lors que les lecteurs historiques ont ete migres vers `LlmCallLogOperationalMetadataModel.executed_provider`. Les logs ne doivent plus porter aucun champ provider ambigu ou de compatibilite.
63. **AC63 - Migration des lectures historiques de provider** : tous les services, dashboards, agregats, endpoints admin et scripts qui lisaient encore `provider`, `provider_compat` ou un fallback provider historique doivent lire exclusivement `operational_metadata.executed_provider`, ou une valeur calculee explicitement dans une couche de migration ponctuelle non persistee.
64. **AC64 - Suppression du remapping implicite `provider=`** : `LlmCallLogModel.__init__` ne doit plus accepter `provider=` comme alias d entree. Toute tentative d instancier un log avec `provider=` doit echouer, afin d eviter la reintroduction silencieuse d un contrat legacy.
65. **AC65 - Suppression complete de `llm_compatibility.py` si vide** : lorsque toutes les compatibilites legacy LLM sont supprimees, le module `llm_compatibility.py` doit disparaitre. Le perimetre canonique ne doit plus exposer de `compatibility_specs`, sauf si une compatibilite restante est explicitement hors perimetre de cette story.
66. **AC66 - Mise a jour du perimetre canonique sans statut `canonical_with_compat_layer`** : `llm_canonical_perimeter.py` ne doit plus referencer de compatibilite legacy pour les modeles LLM supprimes. Les statuts des tables doivent etre `canonical`, et aucun champ `compatibility_specs` ne doit subsister pour les tables assainies.
67. **AC67 - Documentation runtime sans mention de source legacy** : les documents `llm-runtime-source-of-truth.md`, `llm-canonical-consumption-rebuild.md`, `llm-release-runbook.md` et toute documentation admin LLM doivent etre nettoyes de toute mention laissant entendre qu un champ legacy peut encore servir de source runtime nominale.
68. **AC68 - Tests anti-reintroduction legacy exhaustifs** : un test de perimetre doit echouer si l un des noms legacy supprimes reapparait dans les modeles SQLAlchemy, services runtime, serializers admin, seeds, migrations recentes ou tests nominaux : `execution_config`, `output_contract_ref`, `fallback_use_case`, `provider_compat`, `fallback_use_case_key`, `allowed_persona_ids`, `reasoning_effort` dans prompt version, `max_output_tokens` dans prompt version, `temperature` dans prompt version.
69. **AC69 - Aucun fallback silencieux vers legacy** : le runtime, l admin et les services d observabilite ne doivent plus utiliser de logique de type `getattr(..., legacy_name, default)`, fallback silencieux, alias, mapping ou conversion destinee a absorber un ancien contrat legacy. Tout payload legacy doit echouer explicitement ou passer par une migration dediee.
