...
- **Correctif affichage frontend** : les lignes de quotas partagent le même `feature_code`; la liste React utilise désormais une clé composée `feature_code + period` pour éviter qu'une seule période visible n'écrase les autres au rendu.
- **Correctif backend quotas multi-fenêtres** : l'agrégation API distingue désormais les quotas `astrologer_chat` par période (`day`, `week`, `month`) même lorsqu'ils partagent le même `quota_key` (`tokens`), évitant l'écrasement des entrées dans la réponse JSON.
