...
- Correctifs de stabilisation du 2026-04-26 apres execution de la suite complete : restauration de points de compatibilite locaux dans `app/services/billing/service.py` (alias `settings`, `_BILLING_QUOTA_FEATURE`, `_to_plan_data`, `_to_stripe_subscription_data`) pour absorber les patch targets/tests historiques sans recreer de shim de chemin, correction des references de tests encore orientees vers les anciens chemins `quota_*` et `stripe_webhook_service.py`, puis neutralisation explicite de la gate `HoroscopeDailyEntitlementGate` dans les suites d integration prediction qui ne valident pas l entitlement.
...
- La facade `app/services/billing/service.py` conserve en plus un lot borne de compatibilite intra-module pour les routeurs et tests historiques encore relies a des symboles prives (`_BILLING_QUOTA_FEATURE`, `_to_plan_data`, `_to_stripe_subscription_data`) ; aucun ancien chemin de module n a ete reintroduit et les patch targets nominaux restent le package canonique `app.services.billing.service`.
...
- Les suites `test_daily_prediction_flat_day.py` et `test_daily_prediction_qa.py` neutralisent maintenant explicitement `HoroscopeDailyEntitlementGate.check_and_get_variant`, sur le meme pattern que les autres suites routeur prediction, afin d isoler la projection publique et d eviter les regressions 403 non pertinentes pour ces scenarios.
...
- Validation complementaire executee dans le venv apres stabilisation post-suite complete : `pytest -q backend/app/tests/integration/test_billing_api.py backend/app/tests/integration/test_admin_actions_api.py -k "billing_plans or user_detail_lists_daily_weekly_and_monthly_quotas"` (`5 passed`), `pytest -q backend/app/tests/integration/test_daily_prediction_flat_day.py backend/app/tests/integration/test_daily_prediction_qa.py` (`22 passed`), `pytest -q backend/app/tests/unit/test_billing_service.py backend/app/tests/unit/test_story_70_22_entitlement_structure_guard.py backend/app/tests/unit/test_stripe_webhook_local_dev_assets.py backend/app/tests/unit/test_thematic_consultation_entitlement_gate.py` (`19 passed`), puis `ruff check` cible vert sur les fichiers touches.
...
- 2026-04-26 - Stabilisation post-refactor suite a la campagne `pytest -q` complete : restauration des compatibilites intra-module bornees dans `billing/service.py`, realignement de plusieurs tests vers les nouveaux chemins canoniques et neutralisation explicite de la gate entitlement dans les suites prediction concernees.
