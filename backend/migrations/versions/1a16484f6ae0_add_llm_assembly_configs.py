...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["feature_template_ref"], ["llm_prompt_versions.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(["persona_ref"], ["llm_personas.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(
            ["subfeature_template_ref"], ["llm_prompt_versions.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    with op.batch_alter_table("llm_assembly_configs", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_llm_assembly_configs_feature"), ["feature"], unique=False
        )
        batch_op.create_index(
            batch_op.f("ix_llm_assembly_configs_locale"), ["locale"], unique=False
        )
        batch_op.create_index(batch_op.f("ix_llm_assembly_configs_plan"), ["plan"], unique=False)
        batch_op.create_index(
            batch_op.f("ix_llm_assembly_configs_status"), ["status"], unique=False
        )
        batch_op.create_index(
            batch_op.f("ix_llm_assembly_configs_subfeature"), ["subfeature"], unique=False
        )
        batch_op.create_index(
            batch_op.f("ix_llm_assembly_configs_created_at"), ["created_at"], unique=False
        )
        batch_op.create_index(
            "ix_llm_assembly_config_active_unique",
            [
                "feature",
                sa.text("coalesce(subfeature, '')"),
                sa.text("coalesce(plan, '')"),
                "locale",
            ],
            unique=True,
