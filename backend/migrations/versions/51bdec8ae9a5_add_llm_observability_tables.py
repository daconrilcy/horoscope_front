...
def upgrade() -> None:
    op.create_table('llm_call_logs',
        ...
        sa.ForeignKeyConstraint(['assembly_id'], ['llm_assembly_configs.id'], ),
        sa.ForeignKeyConstraint(['persona_id'], ['llm_personas.id'], ),
    ...
    with op.batch_alter_table('llm_call_logs', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_llm_call_logs_request_id'), ['request_id'], unique=False)
        batch_op.create_index('ix_llm_call_logs_prompt_v_timestamp', ['prompt_version_id', 'timestamp'], unique=False)
        batch_op.create_index('ix_llm_call_logs_status_timestamp', ['validation_status', 'timestamp'], unique=False)
        batch_op.create_index('ix_llm_call_logs_use_case_timestamp', ['use_case', 'timestamp'], unique=False)

    op.create_table('llm_replay_snapshots',
        sa.Column('id', sa.UUID(), nullable=False),
