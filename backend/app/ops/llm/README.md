...
- Le registre `TRANSITION_WRAPPERS.md` a ete retire : plus de shims `app.llm_orchestration` ; le runtime LLM nominal vit sous `app.domain.llm.*`, `app.domain.llm.runtime.*`, `app.infra.*` et `app.ops.llm.*`.
