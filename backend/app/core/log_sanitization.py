"""Fournit les garde-fous de sanitisation pour les logs applicatifs."""
...
_SENSITIVE_DOMAIN_MARKERS = ("birth", "latitude", "longitude")
_CONTENT_FIELDS = {"content", "message", "messages", "raw_output", "structured_output"}
_SAFE_FIELDS = {
    "use_case",
    "locale",
    "request_id",
...
def sanitize_request_for_logging(
    ...
    **extra: Any,
    ...
    """
    Construit un payload de log applicatif sans données sensibles.
    """
    result: Dict[str, Any] = {
        "operation": operation,
        "user_id": str(user_id),
...
def sanitize_messages_for_logging(messages: list[Dict[str, Any]]) -> list[Dict[str, Any]]:
    """
    Sanitize des messages utilisateur pour la journalisation.
    """
    result = []
    for msg in messages:
