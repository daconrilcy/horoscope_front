...
def admin_token():
    ...
    response = client.post(
        "/v1/auth/login", json={"email": "admin-support@example.com", "password": "admin123"}
    )
...
def test_list_tickets_and_flagged_content(admin_token):
    ...
    # 1. List Tickets
    response = client.get(
        "/v1/admin/support/tickets", headers={"Authorization": f"Bearer {admin_token}"}
    )
    ...
    assert response.status_code == 200
    assert response.json()["data"]["audit_trail"][0]["action"] == "support_ticket_action"

    # 3. List Flagged Content
    response = client.get(
        "/v1/admin/support/flagged-content", headers={"Authorization": f"Bearer {admin_token}"}
    )
    assert response.status_code == 200
    assert len(response.json()["data"]) >= 1
    ...
    with open_app_test_db_session() as db:
        ...
        audit_flagged = db.scalar(
            select(AuditEventModel).where(AuditEventModel.action == "flagged_content_reviewed")
        )
