...
from app.prediction.persistence_service import PredictionPersistenceService, SaveResult
from app.prediction.schemas import EngineOutput, EffectiveContext
...
def test_create_new_run(db_session: Session, seed_data):
    ...
    engine_output = EngineOutput(
        ...
        category_scores={
            "love": {"note_20": 15, "raw_score": 0.5, "normalized_score": 0.6, "power": 0.7, "volatility": 0.1},
            "work": {"note_20": 18, "raw_score": 0.8, "normalized_score": 0.9, "power": 0.5, "volatility": 0.2}
        },
        turning_points=[],
        time_blocks=[]
    )
...
def test_reuse_existing_hash(db_session: Session, seed_data):
    ...
    # First save
    res1 = service.save(engine_output, user_id, local_date, seed_data["version_id"], seed_data["ruleset_id"], db_session)
    assert res1.was_reused is False
    
    # Second save same hash
    res2 = service.save(engine_output, user_id, local_date, seed_data["version_id"], seed_data["ruleset_id"], db_session)
...
def test_rank_correct(db_session: Session, seed_data):
    ...
    engine_output = EngineOutput(
        run_metadata={},
        effective_context=EffectiveContext(
            house_system_requested="placidus", house_system_effective="placidus", timezone="UTC", input_hash="hash_rank"
        ),
        category_scores={
    ...
    result = service.save(engine_output, user_id, local_date, seed_data["version_id"], seed_data["ruleset_id"], db_session)
...
def test_rank_tiebreak_sort_order(db_session: Session, seed_data):
    ...
    engine_output = EngineOutput(
        run_metadata={},
        effective_context=EffectiveContext(
            house_system_requested="placidus", house_system_effective="placidus", timezone="UTC", input_hash="hash_tie"
        ),
        category_scores={
            "love": {"note_20": 15},
            "work": {"note_20": 15}
...
def test_turning_points_persisted(db_session: Session, seed_data):
    ...
    engine_output = EngineOutput(
        run_metadata={},
        effective_context=EffectiveContext(
            house_system_requested="placidus", house_system_effective="placidus", timezone="UTC", input_hash="hash_tp"
        ),
        category_scores={},
        turning_points=[
            {
...
def test_time_blocks_persisted(db_session: Session, seed_data):
    ...
    engine_output = EngineOutput(
        run_metadata={},
        effective_context=EffectiveContext(
            house_system_requested="placidus", house_system_effective="placidus", timezone="UTC", input_hash="hash_blocks"
        ),
        category_scores={},
        turning_points=[],
        time_blocks=[
...
def test_transaction_rollback(db_session: Session, seed_data):
    ...
    engine_output = EngineOutput(
        run_metadata={},
        effective_context=EffectiveContext(
            house_system_requested="placidus", house_system_effective="placidus", timezone="UTC", input_hash="hash_fail"
        ),
        category_scores={"love": {"note_20": 15}},
        turning_points=[],
        time_blocks=[]
    ...
    with patch.object(service, "_save_scores", side_effect=RuntimeError("injected failure")):
        with pytest.raises(RuntimeError, match="injected failure"):
            service.save(engine_output, user_id, local_date, seed_data["version_id"], seed_data["ruleset_id"], db_session)

    # Rollback simulates what the caller (endpoint/use-case) does on error
    db_session.rollback()
    run = db_session.scalar(select(DailyPredictionRunModel).where(DailyPredictionRunModel.input_hash == "hash_fail"))
    assert run is None
...
def test_new_run_on_hash_change(db_session: Session, seed_data):
    ...
    def make_output(hash_val, local_date):
        return EngineOutput(
            run_metadata={},
            effective_context=EffectiveContext(
                house_system_requested="placidus", house_system_effective="placidus", timezone="UTC", input_hash=hash_val
            ),
            category_scores={"love": {"note_20": 15}},
            turning_points=[],
            time_blocks=[]
    ...
    output_b, date_b = make_output("hash_b", date(2026, 3, 8))  # different date avoids unique constraint

    res1 = service.save(output_a, user_id, date_a, seed_data["version_id"], seed_data["ruleset_id"], db_session)
    res2 = service.save(output_b, user_id, date_b, seed_data["version_id"], seed_data["ruleset_id"], db_session)
...
def test_scores_persisted(db_session: Session, seed_data):
    ...
    engine_output = EngineOutput(
        run_metadata={},
        effective_context=EffectiveContext(
            house_system_requested="placidus", house_system_effective="placidus", timezone="UTC", input_hash="hash_scores"
        ),
        category_scores={
            "love": {"note_20": 15, "raw_score": 0.5, "normalized_score": 0.6, "power": 0.7, "volatility": 0.1},
            "work": {"note_20": 18, "raw_score": 0.8, "normalized_score": 0.9, "power": 0.5, "volatility": 0.2},
...
def test_idempotent_double_save(db_session: Session, seed_data):
    ...
    engine_output = EngineOutput(
        run_metadata={},
        effective_context=EffectiveContext(
            house_system_requested="placidus", house_system_effective="placidus", timezone="UTC", input_hash="hash_idempotent"
        ),
        category_scores={"love": {"note_20": 15}},
        turning_points=[],
        time_blocks=[]
