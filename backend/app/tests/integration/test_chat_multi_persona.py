...
def test_chat_anti_doublon_invariant():
    ...
    with SessionLocal() as db:
        ...
        # Should fail with IntegrityError when trying to create second active conversation for same user/persona
...
def test_get_or_create_active_conversation_concurrency():
    ...
    with SessionLocal() as db:
        ...
        count = db.scalar(select(func.count(ChatConversationModel.id)).where(
