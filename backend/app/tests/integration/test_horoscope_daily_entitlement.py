...
def test_resolve_horoscope_daily_variant_by_plan(db_session: Session, seeded_catalog):
    ...
    sub_basic = UserSubscriptionModel(user_id=user_basic.id, plan_id=plans["basic"], status="active")
    sub_premium = UserSubscriptionModel(user_id=user_premium.id, plan_id=plans["premium"], status="active")
    ...
    # 1. Free -> summary_only
    snapshot_free = EffectiveEntitlementResolverService.resolve_b2c_user_snapshot(db_session, app_user_id=user_free.id)
    ...
    gate_res_free = HoroscopeDailyEntitlementGate.check_and_get_variant(db_session, user_id=user_free.id)
    assert gate_res_free.variant_code == "summary_only"
    
    # 2. Basic -> full
    snapshot_basic = EffectiveEntitlementResolverService.resolve_b2c_user_snapshot(db_session, app_user_id=user_basic.id)
    assert snapshot_basic.entitlements["horoscope_daily"].variant_code == "full"
    
    gate_res_basic = HoroscopeDailyEntitlementGate.check_and_get_variant(db_session, user_id=user_basic.id)
    assert gate_res_basic.variant_code == "full"
    
    # 3. Premium -> full
    snapshot_premium = EffectiveEntitlementResolverService.resolve_b2c_user_snapshot(db_session, app_user_id=user_premium.id)
    assert snapshot_premium.entitlements["horoscope_daily"].variant_code == "full"
    
    gate_res_premium = HoroscopeDailyEntitlementGate.check_and_get_variant(db_session, user_id=user_premium.id)
