...
def test_get_daily_prediction_passes_correct_variant_to_narrator(
    ...
    db_session.add(UserSubscriptionModel(user_id=user_free.id, plan_id=free_plan_id, status="active"))
    db_session.commit()
    ...
    with patch("app.api.v1.routers.predictions.settings") as mock_settings_router, \
    ...
    db_session.add(UserSubscriptionModel(user_id=user_basic.id, plan_id=basic_plan_id, status="active"))
    db_session.commit()
