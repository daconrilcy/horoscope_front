...
@patch("app.services.effective_entitlement_resolver_service.EffectiveEntitlementResolverService._get_next_plan")
def test_compute_upgrade_hints_free_user(mock_next_plan, db_session):
    ...
    snapshot = _make_snapshot(
        ...
        entitlements={
            ...
            "natal_chart_short": _make_access(granted=True), # non-restricted
        }
    )
    ...
    assert chat_hint.benefit_key == "upgrade.astrologer_chat.unlock"
    assert chat_hint.cta_variant == "banner"
    assert chat_hint.priority == 10


def test_compute_upgrade_hints_no_next_plan(db_session):
    with patch("app.services.effective_entitlement_resolver_service.EffectiveEntitlementResolverService._get_next_plan") as mock_next:
...
def test_get_next_plan_logic(db_session):
    ...
    from app.infra.db.models.product_entitlements import PlanCatalogModel, Audience
    
    # Seed local mock DB
    p1 = BillingPlanModel(code="free", display_name="Free", monthly_price_cents=0, daily_message_limit=0)
    p2 = BillingPlanModel(code="basic", display_name="Basic", monthly_price_cents=900, daily_message_limit=0)
    p3 = BillingPlanModel(code="premium", display_name="Premium", monthly_price_cents=2900, daily_message_limit=0)
    ...
    # Unknown plan (ex: "none") -> returns first (free if price 0, or basic if sorted)
