...
def create_mock_baseline(cat_code, mean, std, p10, p50, p90, g_type="day", g_val="all"):
    return PersistedUserBaseline(
        id=1, user_id=1, category_id=1, category_code=cat_code,
        reference_version_id=1, ruleset_id=1, house_system_effective="placidus",
        window_days=365, window_start_date=date(2025, 3, 11), window_end_date=date(2026, 3, 11),
        mean_raw_score=mean, std_raw_score=std,
        mean_note_20=10.0, std_note_20=2.0,
        p10=p10, p50=p50, p90=p90,
        sample_size_days=365, computed_at=datetime.now(UTC),
        granularity_type=g_type, granularity_value=g_val
    )
...
def test_relative_scoring_nominal_granular():
    ...
    baselines = {
        "work": {
            ...
            "slot": create_mock_baseline("work", 2.0, 0.5, 1.5, 2.0, 2.5, g_type="slot", g_val="morning"),
            "season": create_mock_baseline("work", 0.5, 0.5, 0.0, 0.5, 1.0, g_type="season", g_val="spring")
        }
    }
    raw_scores = {"work": 1.5}
...
def test_relative_scoring_rarity_fallback():
    ...
    baselines = {
        "work": {
            "day": create_mock_baseline("work", 1.0, 0.0, 1.0, 1.0, 1.0)
        }
    }
    raw_scores = {"work": 1.5}
