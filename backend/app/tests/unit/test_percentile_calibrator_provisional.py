...
def test_day_relative_gives_spread():
    ...
    assert (
        results["love"] < results["work"] < results["health"] < results["money"] < results["energy"]
    )
...
def test_mixed_calibration():
    ...
    real_cal = CalibrationData(
        p05=-2, p25=-1, p50=0, p75=1, p95=2, sample_size=100, calibration_label="real"
    )
