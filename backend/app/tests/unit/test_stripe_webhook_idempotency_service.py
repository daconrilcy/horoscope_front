...
def make_mock_event(event_id: str, event_type: str, stripe_object_id: str | None = "obj_123") -> stripe.Event:
...
def test_mark_processed_updates_status(db_session: Session):
    ...
    record = db_session.query(StripeWebhookEventModel).filter_by(stripe_event_id="evt_proc_ok").first()
...
def test_mark_failed_updates_status_and_error(db_session: Session):
    ...
    record = db_session.query(StripeWebhookEventModel).filter_by(stripe_event_id="evt_proc_fail").first()
