...
def test_consume_called_once_per_quota(db_session):
    quota1 = QuotaDefinition(quota_key="q1", quota_limit=5, period_unit="day", period_value=1, reset_mode="calendar")
    quota2 = QuotaDefinition(quota_key="q2", quota_limit=10, period_unit="month", period_value=1, reset_mode="calendar")
    ...
    with patch.object(EntitlementService, "get_feature_entitlement", return_value=entitlement), \
         patch.object(QuotaUsageService, "consume", side_effect=[mock_state1, mock_state2]) as mock_consume:
        result = ChatEntitlementGate.check_and_consume(db_session, user_id=1)
