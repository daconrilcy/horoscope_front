...
def test_timeout_raises_on_slow_run(service, mock_deps, db_session):
    ...
    service._compute_with_timeout = MagicMock(
        side_effect=DailyPredictionServiceError("timeout", "Calcul trop long — service temporairement dégradé")
    )
...
def test_latest_run_fallback_found(service, db_session):
    ...
    r1 = DailyPredictionRunModel(
        user_id=1, local_date=date(2024, 1, 1), timezone="UTC",
        reference_version_id=1, ruleset_id=1, input_hash="h1"
    )
    r2 = DailyPredictionRunModel(
        user_id=1, local_date=date(2024, 1, 5), timezone="UTC",
        reference_version_id=1, ruleset_id=1, input_hash="h2"
    )
...
def test_log_includes_duration(caplog, service, mock_deps, db_session):
    ...
    service._compute_with_timeout = MagicMock(return_value=MagicMock(
        turning_points=[],
