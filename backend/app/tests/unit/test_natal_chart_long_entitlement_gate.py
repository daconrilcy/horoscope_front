...
def test_canonical_quota_path_consumes(db_session):
    ...
    with patch("app.services.entitlement_service.EntitlementService.get_feature_entitlement",
...
def test_quota_exceeded_raises_natal_error(db_session):
    ...
    with patch("app.services.entitlement_service.EntitlementService.get_feature_entitlement",
               return_value=entitlement), \
         patch("app.services.quota_usage_service.QuotaUsageService.consume",
               side_effect=QuotaExhaustedError(quota_key="interpretations", used=1, limit=1, feature_code="natal_chart_long")):
...
def test_no_legacy_quota_service_called(db_session):
    # This is a bit indirect, but we check that only EntitlementService and QuotaUsageService are used
    entitlement = make_entitlement()
    mock_state = MagicMock(used=1, remaining=0, quota_limit=1, window_end=None)
