...
class EventDetector:
    ...
    def _detect_moon_ingress(self, steps: list[StepAstroState]) -> list[AstroEvent]:
        ...
        for step in steps:
            ...
            if prev_sign is not None and moon.sign_code != prev_sign:
                detected.append(self._create_event(
                    "moon_sign_ingress", step.ut_jd, step.local_time,
                    "Moon", None, None, 0.0, {"from_sign": prev_sign, "to_sign": moon.sign_code}
                ))
    ...
    def _detect_asc_sign_change(self, steps: list[StepAstroState]) -> list[AstroEvent]:
        ...
        for step in steps:
            ...
            if prev_sign is not None and sign_code != prev_sign:
                detected.append(self._create_event(
                    "asc_sign_change", step.ut_jd, step.local_time,
                    "Asc", None, None, 0.0, {"from_sign": prev_sign, "to_sign": sign_code}
                ))
    ...
    def _create_planetary_hour_event(
        ...
        return self._create_event(
            ...
            ruler, None, None, 0.0, {"hour_number": hour_num}
        )
