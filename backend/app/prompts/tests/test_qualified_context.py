...
from app.prompts.common_context import QualifiedContext, CommonContextBuilder, PromptCommonContext
...
def test_is_degraded():
    ctx = QualifiedContext(
        payload=PromptCommonContext(
            ...
            today_date="d", use_case_name="n", use_case_key="k"
        ),
        source="db",
...
async def test_build_full_context(db):
    ...
    with patch("app.services.user_birth_profile_service.UserBirthProfileService.get_for_user") as mock_profile, \
         patch("app.services.persona_config_service.PersonaConfigService.get_active") as mock_persona, \
         patch("app.services.user_natal_chart_service.UserNatalChartService.get_latest_for_user") as mock_chart:
        
        mock_profile.return_value = MagicMock(birth_time="12:00", birth_place="Paris")
        mock_persona.return_value = MagicMock(display_name="A", response_style="S", tone="T", prudence_level="L", to_prompt_line=lambda: "desc")
        
        # Ensure model_dump returns a real dict
        mock_chart.return_value.result.model_dump.return_value = {"planets": []}
        
        # Test 1: Full
        res = CommonContextBuilder.build(user_id=1, use_case_key="natal_interpretation", period="daily", db=db)
