...
LEGACY_COMPAT_PROMPT_CONFIGS: dict[str, dict[str, Any]] = {
    ...
    "natal-long-free": {
        "developer_prompt": "Génère un horoscope natal long (compat legacy).",
...
def build_legacy_compat_use_case_config(use_case_key: str) -> UseCaseConfig | None:
    """
    Bounded compatibility config for legacy-only runtime paths.

    This intentionally replaces the old legacy stubs module with a single explicit
...
def resolve_legacy_model(use_case_key: str, fallback_model: str | None = None) -> str:
    """
    Legacy model resolution kept only for bounded compatibility paths.

    Order of resolution:
    1. OS environment variable (OPENAI_ENGINE_{UC})
    2. Legacy model override (LLM_MODEL_OVERRIDE_{UC})
    3. fallback_model (usually from DB config or legacy stub)
