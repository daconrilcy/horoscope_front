...
async def test_simulate_rate_limit():
    ...
    try:
        ...
        assert "rate_limit" in str(excinfo.value).lower() or "rate limit" in str(excinfo.value).lower()
    finally:
...
async def test_simulate_server_error():
    ...
    try:
        with pytest.raises(Exception) as excinfo:
            await manager.execute_with_resilience(messages=[], model="gpt-4o")
        assert "server error" in str(excinfo.value).lower() or "upstream_error" in str(excinfo.value).lower()
    finally:
        simulation_error_ctx.reset(token)
