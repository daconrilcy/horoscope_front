...
from app.llm_orchestration.semantic_invariants_registry import (
...
def test_deprecated_use_case_mapping_matches_registry_json() -> None:
    data_path = Path(__file__).resolve().parents[1] / "data" / "prompt_governance_registry.json"
...
def test_incomplete_governed_exception_rejected() -> None:
    base = json.loads(
        (
            Path(__file__).resolve().parents[1] / "data" / "prompt_governance_registry.json"
