...
async def test_execution_profile_assembly_ref(db):
    ...
    fv = LlmPromptVersionModel(
        id=uuid.uuid4(), use_case_key="feat_uc", developer_prompt="PROMPT",
        status=PromptStatus.PUBLISHED, model="gpt-4o", created_by="test"
    )
    db.add(fv)
    uc = LlmUseCaseConfigModel(key="feat_uc", display_name="UC", description="test", safety_profile="astrology")
    db.add(uc)
