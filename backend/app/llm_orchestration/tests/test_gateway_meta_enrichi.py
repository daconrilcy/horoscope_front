...
def test_build_result_enriched_telemetry():
    ...
    provider_res = GatewayResult(
        ...
        raw_output="...",
        usage=UsageInfo(),
        meta=GatewayMeta(latency_ms=0, model="gpt-4o"),
    )
    ...
    final = gateway._build_result(validation, plan, recovery, 150, request, qualified_ctx)
...
def test_build_result_test_fallback_path():
    ...
    request = LLMExecutionRequest(
        ...
        request_id="r",
        trace_id="t",
        flags=ExecutionFlags(test_fallback_active=True),
    )

    validation = ValidationResult(valid=True, parsed={}, normalizations_applied=[])

    final = gateway._build_result(validation, plan, recovery, 10, request)
