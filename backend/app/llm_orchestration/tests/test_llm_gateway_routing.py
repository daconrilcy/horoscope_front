...
@pytest.mark.asyncio
async def test_routing_v1_when_flag_false(monkeypatch):
    # Arrange
    monkeypatch.setattr(ai_engine_settings, "llm_orchestration_v2", False)
...
@pytest.mark.asyncio
async def test_routing_v2_when_flag_true(monkeypatch):
    # Arrange
    # Use monkeypatch to set flag to true
    monkeypatch.setattr(ai_engine_settings, "llm_orchestration_v2", True)
