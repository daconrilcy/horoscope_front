"""Service applicatif de consommation canonique LLM pour admin et exports."""
...
ConsumptionView = Literal["user", "subscription", "feature"]


class CanonicalConsumptionFilters(BaseModel):
    """Filtres de lecture du read model canonique de consommation."""
...
class CanonicalConsumptionAggregate(BaseModel):
    """Ligne agrégée du read model de consommation canonique."""
...
class CanonicalConsumptionDrilldownEntry(BaseModel):
    """Ligne safe de drilldown admin sur les appels LLM."""
...
class CanonicalConsumptionExportRow(BaseModel):
    """Ligne exportable des generations admin avec taxonomie canonique."""
...
class LlmCanonicalConsumptionService:
    """Service de read model, drilldown et export de consommation LLM."""
    ...
    def get_aggregates(
        ...
        *,
        ...
        """Lit les agrégats persistés du read model canonique."""
        if refresh:
    ...
    @staticmethod
    def get_average_latency_index(
        db: Session,
        *,
        filters: CanonicalConsumptionFilters,
        view: ConsumptionView,
    ) -> dict[tuple[Any, ...], float]:
        """Calcule les latences moyennes exactes par vue admin."""
        ...
        return {
            key: round(sum(latencies) / len(latencies), 2) if latencies else 0.0
            for key, latencies in grouped_latencies.items()
        }

    @staticmethod
    def get_drilldown_entries(
        db: Session,
        *,
        view: ConsumptionView,
        granularity: Granularity,
        ...
        subfeature: str | None = None,
        limit: int = 50,
    ) -> list[CanonicalConsumptionDrilldownEntry]:
        """Produit les lignes de drilldown admin sans exposer le log brut."""
        ...
        period_end = LlmCanonicalConsumptionService._resolve_bucket_end(
            granularity=granularity,
            period_start_utc=period_start,
        )
        ...
        rows = db.execute(stmt.order_by(desc(LlmCallLogModel.timestamp)).limit(capped_limit)).all()
        return [
            CanonicalConsumptionDrilldownEntry(
                request_id=str(row.request_id),
                timestamp=row.timestamp,
                feature=row.feature,
                subfeature=row.subfeature,
                provider=row.executed_provider,
                active_snapshot_version=row.active_snapshot_version,
    ...
    def get_export_rows(
        ...
        *,
        ...
        to_utc: datetime | None = None,
        limit: int = 5000,
    ) -> list[CanonicalConsumptionExportRow]:
        """Construit les lignes exportables admin de consommation LLM."""
        stmt = (
            select(
                LlmCallLogModel.id,
        ...
        if from_utc is not None:
            stmt = stmt.where(LlmCallLogModel.timestamp >= from_utc)
        if to_utc is not None:
            stmt = stmt.where(LlmCallLogModel.timestamp <= to_utc)

        rows = db.execute(stmt).all()
        return [LlmCanonicalConsumptionService._build_export_row(row) for row in rows]
    ...
    @staticmethod
    def _build_export_row(row: Any) -> CanonicalConsumptionExportRow:
        ...
        return CanonicalConsumptionExportRow(
            ...
            tokens_completion=(
                int(row.tokens_completion) if row.tokens_completion is not None else None
            ),
    ...
    def _resolve_bucket_end(*, granularity: Granularity, period_start_utc: datetime) -> datetime:
        ...
        return period_start_utc.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(
            days=1
        )
