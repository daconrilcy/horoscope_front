...
class UserPredictionBaselineService:
    ...
    def generate_baseline(
        ...
        while current_date <= end_date:
            ...
            if house_system_effective is None:
                house_system_effective = bundle.core.effective_context.house_system_effective or "placidus"

            # 1. Day level accumulation
            for cat_code, scores in bundle.core.category_scores.items():
                accumulators[(V3Granularity.DAY, "all", cat_code)].append(float(scores["raw_score"]))
                accumulators_notes[(V3Granularity.DAY, "all", cat_code)].append(int(scores["note_20"]))

            # 2. Season/Month level accumulation
            season = self._get_season(current_date)
            month = f"month_{current_date.month}"
            for cat_code, scores in bundle.core.category_scores.items():
                accumulators[(V3Granularity.SEASON, season, cat_code)].append(float(scores["raw_score"]))
                accumulators_notes[(V3Granularity.SEASON, season, cat_code)].append(int(scores["note_20"]))
                accumulators[(V3Granularity.MONTH, month, cat_code)].append(float(scores["raw_score"]))
                accumulators_notes[(V3Granularity.MONTH, month, cat_code)].append(int(scores["note_20"]))
            ...
            if bundle.v3_core and bundle.v3_core.time_blocks:
                for block in bundle.v3_core.time_blocks:
                    ...
                    for cat_code in block.dominant_themes:
                        ...
                        accumulators_notes[(V3Granularity.SLOT, slot, cat_code)].append(int(block.intensity))
    ...
    def _percentile(self, sorted_data: list[float], percentile: float) -> float:
        ...
        low = int(idx)
        high = min(low + 1, n - 1)
        if low == high: return sorted_data[low]
        weight = idx - low
