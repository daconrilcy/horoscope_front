...
class ConsultationPrecheckService:
    ...
    def _detect_safeguard_issue(question: str | None) -> SafeguardIssue | None:
        """Détecte les questions sensibles qui imposent un garde-fou métier."""
        if not question:
            return None
    ...
    def precheck(
        ...
        """Évalue la complétude et les garde-fous d'une consultation canonique."""
        ...
        is_relationship_consultation = request.consultation_type == "relationship"
        ...
        if is_relationship_consultation:
            if not request.other_person:
                ...
                if status != ConsultationStatus.blocked:
                    ...
                    status = ConsultationStatus.degraded
                    precision = PrecisionLevel.medium
                    available_modes.append("relationship_user_only")
            else:
                if not request.other_person.birth_time_known:
                    ...
                    available_modes.append("relationship_full")
