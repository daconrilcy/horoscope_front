...
class PromptRenderer:
    ...
    def render(
        ...
        missing_legacy = [
            v for v in required_variables if f"{{{{{v}}}}}" in template and v not in variables
        ]
        if missing_legacy:
        ...
        for p_name in found_placeholders:
            ...
            if p_def.classification == "required":
                msg = (
                    f"Required placeholder '{{{{{p_name}}}}}' not resolved for feature '{feature}'"
                )
                if (
                    feature in PLACEHOLDER_POLICY.blocking_features
                    or feat_key in PLACEHOLDER_POLICY.blocking_features
                ):
                    raise PromptRenderError(
                        msg, details={"placeholder": p_name, "feature": feature}
                    )
                else:
                    logger.error(
                ...
                logger.info(
                    "placeholder_fallback_used placeholder=%s fallback=%s", p_name, fallback_val
                )
