...
from app.infra.db.models.llm_prompt import LlmPromptVersionModel, LlmUseCaseConfigModel, PromptStatus
...
def seed_66_15_convergence():
    with Session(engine) as db:
        ...
        for key, p in profiles.items():
            ...
            existing = db.execute(select(LlmExecutionProfileModel).where(LlmExecutionProfileModel.name == p.name)).scalar_one_or_none()
            if not existing:
                db.add(p)
            else:
        ...
        assemblies = [
            # NATAL
            ("natal", "natal_interpretation", "premium", "natal_interpretation", profiles["premium"].id),
        ...
        for f, sf, p, uc_key, prof_id in assemblies:
            ...
            # Avoid duplicates
            existing = db.execute(select(PromptAssemblyConfigModel).where(
            ...
            if not existing:
                config = PromptAssemblyConfigModel(
                    ...
                    execution_config={"model": v.model, "max_output_tokens": v.max_output_tokens}, # compatibility
                    status=PromptStatus.PUBLISHED,
                    created_by="system_66_15"
                )
                db.add(config)
