...
ALL_PROMPT_CONFIGS = [
    ...
    *[
        {
            "use_case_key": c["use_case_key"],
            "developer_prompt": c["developer_prompt"],
            "model": "gpt-5",
            "temperature": 0.5,
            "max_output_tokens": 32000,
            "reasoning_effort": "low",
        ...
        for c in THEMATIC_PROMPT_CONFIGS
    ],
]
        ...
        for prompt_cfg in ALL_PROMPT_CONFIGS:
            ...
            if published_versions:
                for p in published_versions:
                    if (
                        p.developer_prompt == prompt_cfg["developer_prompt"]
                        and p.model == prompt_cfg["model"]
                        and p.temperature == prompt_cfg["temperature"]
                        and p.max_output_tokens == prompt_cfg["max_output_tokens"]
                        and p.reasoning_effort == prompt_cfg["reasoning_effort"]
                        and p.verbosity == prompt_cfg["verbosity"]
                    ):
                        logger.info(
