...
def test_basic_final_prompt_renders_editorial_contract_from_published_assembly() -> None:
    """Le prompt Basic final vient de l'assembly publiee et cadre le rapport humain."""
    rendered_prompt = _render_published_basic_developer_prompt()
    prompt_lower = rendered_prompt.casefold()

    assert "theme_astral_prompt_v1" in rendered_prompt
    assert "basic_natal_prompt_payload" in rendered_prompt
    assert "input_data contient basic_natal_prompt_payload" in rendered_prompt
    for required_field in (
        "basic_natal_prompt_payload.sections",
        ...
        assert required_field in rendered_prompt
    for report_instruction in (
        "lecture natale humaine",
        "introduction",
        "themes explicatifs",
        "conclusion",
        "annexe de sources",
        "sources ne doivent jamais devenir le contenu principal",
        "astrologiques en francais",
        "ne copie pas les labels anglais bruts",
    ):
        assert report_instruction in prompt_lower
    for forbidden_phrase in (
        "cette lecture s'appuie uniquement",
        "Ce repere retient",
        "avec une confiance editoriale controlee",
    ):
        assert forbidden_phrase in rendered_prompt
    assert "natal_interpretation" not in rendered_prompt
    assert "natal_interpretation_short" not in rendered_prompt
    assert "chart_json" not in rendered_prompt
    assert "natal_data" not in rendered_prompt
...
def _render_published_basic_developer_prompt() -> str:
    """Rend le prompt developer depuis l'assembly publiee expanded."""
    ...
    try:
        ...
        assembly = db.execute(
            select(PromptAssemblyConfigModel).where(
                ...
                PromptAssemblyConfigModel.plan == "expanded",
                PromptAssemblyConfigModel.status == PromptStatus.PUBLISHED,
            )
        ).scalar_one()
        return assemble_developer_prompt(resolve_assembly(assembly), assembly)
    finally:
        db.close()
