...
def test_theme_natal_slot_claims_are_serialized_by_application_lock(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Deux claims concurrents du meme slot traversent la section critique un par un."""
    start = Barrier(2)
    active_guard = Lock()
    active_claims = 0
    max_active_claims = 0
    ...
    def fake_get_or_create_slot(
        ...
        with active_guard:
            active_claims -= 1
        return SimpleNamespace(id=428), False
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = [
            executor.submit(claim, "client-request-lock-a"),
            executor.submit(claim, "client-request-lock-b"),
        ]
