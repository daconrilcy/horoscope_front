...
def test_v4_payload_compatibility_with_v3_client():
    assembler = PublicPredictionAssembler()
    snapshot = PersistedPredictionSnapshot(
        run_id=1, user_id=1, local_date=date(2026, 3, 18), 
        timezone="UTC", computed_at=datetime.now(UTC),
        input_hash="hash", reference_version_id=1, ruleset_id=1,
        house_system_effective=None, is_provisional_calibration=False,
        calibration_label=None, overall_summary=None, overall_tone=None,
        category_scores=[], turning_points=[], time_blocks=[]
    )
