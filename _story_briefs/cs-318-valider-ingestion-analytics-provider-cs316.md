...
CS-316 est complet côté dépôt, mais l'ingestion réelle reste externe car le provider local est `noop`. La décision produit/technique actuelle est de préparer Plausible en priorité; Matomo n'est pas utilisé et n'est pas la cible de validation à court terme.
...
Vérifier que les événements `/natal` CS-311/CS-316 pourront arriver dans Plausible avec payload redacted, une fois l'environnement Plausible configuré et observable.
...
1. Identifier l'environnement staging ou production où Plausible sera actif.
...
- Ajouter un nouveau provider analytics autre que Plausible.
...
1. Un environnement Plausible observable est identifié, ou son absence est documentée comme blocage externe.
...
4. La preuve d'ingestion cite Plausible, l'environnement, la date, les événements et les champs publics.
...
Le risque principal est organisationnel : l'environnement Plausible peut ne pas être accessible depuis le poste de dev. Dans ce cas, la story doit produire un blocage externe précis au lieu de simuler une validation.
