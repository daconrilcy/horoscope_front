# Schema cible des tables du theme astrologique

Ce schema retire volontairement les tables LLM, personas, interpretations generees et `reference_versions`. Les compteurs actifs/legacy sont documentes dans `.codex-artifacts/astrology-theme-focused-tables.csv`.
...
- `planets`, `signs`, `houses`, `aspects` sont les tables de base du theme.
- `aspects.default_orb_deg`, `planet_profiles.orb_active_deg`, `planet_profiles.orb_peak_deg` et `aspect_profiles.orb_multiplier` portent les donnees principales d'orbes.
- `planet_profiles`, `house_profiles`, `aspect_profiles` enrichissent les tables de base pour le moteur de calcul.
- `astro_points` et `sign_rulerships` servent aux angles et aux maitrises.
- Les tables de poids relient planetes, maisons et angles aux domaines de restitution.
- `category_calibrations`, `calibration_raw_days` et `user_prediction_baselines` sont presentes mais vides en base locale.
