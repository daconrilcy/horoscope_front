# Audit backend generation de prompts LLM (post story 70-15, AC18-AC30)
...
- `app.domain.llm.prompting.renderer`
  - usage nominal: **0**
  - statut: alias de compatibilite temporaire vers `prompt_renderer`
...
- `backend/app/services/ai_engine_adapter.py`
  - raison: compatibilite imports historiques (principalement tests legacy)
...
## Safe to delete next (AC30)

1. `backend/app/domain/llm/prompting/renderer.py`
   - raison: alias de nommage sans valeur fonctionnelle
   - import nominal direct: aucun
   - usage CI bloquant: a verifier apres migration totale des tests legacy
   - statut propose: suppression apres fenetre de transition courte

2. `backend/app/services/ai_engine_adapter.py`
   - raison: shim historique, logique inexistante
   - import nominal production: aucun (reste tests legacy/historique)
   - usage CI bloquant: oui (tests non migres)
   - statut propose: suppression apres migration tests
...
- Le chantier restant est principalement un nettoyage de compatibilite orientee tests/historique, sans blocage architectural de la cible pre-prod.
