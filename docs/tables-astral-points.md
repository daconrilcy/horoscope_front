...
`AstrologyRuntimeReference.astral_points.items` expose des `AstralPointRuntime` avec variantes et alias typés. Le calcul natal sélectionne la variante par défaut, résout une instruction via `AstralPointCalculationResolver`, puis publie les positions dans `NatalResult.points[]`.
...
Le calcul natal brut ne charge pas les keywords ni les profils éditoriaux. Ces données restent réservées à un service d'interprétation séparé qui pourra assembler position, profil et mots-clés sans mélanger calcul astronomique et contenu éditorial.
