...
// Mock useTos
vi.mock('@/features/legal/hooks/useTos', () => ({
  useTos: vi.fn(),
}));
...
describe('TermsOfServicePage', () => {
  ...
  it('devrait afficher le loader pendant le chargement', () => {
    vi.mocked(useTos).mockReturnValue({
      html: undefined,
      meta: {},
      isLoading: true,
      isError: false,
    ...
    render(<TermsOfServicePage />, { wrapper });

    expect(
      screen.getByText(/Chargement des conditions d'utilisation/i)
    ).toBeInTheDocument();
  });

  it('devrait afficher le contenu HTML après chargement', async () => {
    const mockHtml =
      '<div><h2>Section 1</h2><p>Contenu des conditions</p></div>';
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Contenu des conditions')).toBeInTheDocument();
    });
  });
  ...
  it('devrait afficher le titre h1', () => {
    ...
    expect(
      screen.getByRole('heading', { name: "Conditions d'utilisation" })
    ).toBeInTheDocument();
  });
  ...
  it('devrait afficher erreur ApiError avec bouton Réessayer', () => {
    const mockError = new ApiError('Not found', 404, undefined, 'req-123');
    ...
    expect(screen.getByText(/ID de requête : req-123/i)).toBeInTheDocument();
