...
import { billingService, type CheckoutSessionPayload } from '@/shared/api/billing.service';
...
/**
 * Hook React Query pour gérer le checkout Stripe
 * Génère automatiquement l'Idempotency-Key (UUID v4) au clic
...
export function useCheckout(): UseCheckoutResult {
  const { mutateAsync, isPending, error } = useMutation<
    string,
    Error,
    { plan: BillingPlan; idemKey: string; options?: Partial<CheckoutSessionPayload> }
  >({
