...
describe("Admin prompts catalog flow", () => {
  ...
  it("affiche le graphe apres selection feature / abonnement / locale", async () => {
    ...
    expect(screen.getByText("rendered final prompt")).toBeInTheDocument()
  ...
  it("ouvre la modale d edition sur clic du noeud use case", async () => {
    ...
    const useCaseNode = await screen.findByRole("button", { name: /Prompt use case/i })
    await userEvent.click(useCaseNode)
