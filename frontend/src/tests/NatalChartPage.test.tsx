...
describe("NatalChartPage", () => {
  ...
  it("rend une lecture Astral structuree et la precision reduite si le job revient en simplifie", async () => {
    ...
    expect(screen.getByLabelText("Marqueurs clés du portrait astral")).not.toHaveTextContent("Statut")
    ...
    expect(primaryFactIcons[2]).toHaveAttribute("aria-label", "Scorpion")
    const portrait = screen.getByLabelText("Marqueurs clés du portrait astral")
    expect(portrait).toHaveClass("natal-reading-metrics")
    expect(portrait.querySelectorAll(".natal-reading-metrics__item")).toHaveLength(3)
    ...
    expect(container.querySelector(".natal-badge--report-status")?.closest(".natal-reading__summary")).not.toBeNull()
    ...
    expect(renderedProseParagraphs).toHaveLength(3)
    ...
    expect(screen.getByRole("heading", { name: "La formule magique en 4 clés" })).toBeVisible()
