// Domaine API admin prompts: contrats, requetes et hooks TanStack Query associes.
import { useMutation, useQuery, useQueryClient, type QueryClient } from "@tanstack/react-query"

import { API_BASE_URL, apiFetch, parseApiErrorDetails, type ApiResponseEnvelope } from "../client"
...
async function throwAdminPromptsError(response: Response): Promise<never> {
  const error = await parseApiErrorDetails<Record<string, unknown>>(response, {})
  throw new AdminPromptsApiError(error.code, error.message, response.status, error.details)
}
...
export async function listAdminLlmCatalog(
  ...
  try {
    ...
    if (!response.ok) {
      return throwAdminPromptsError(response)
    }
