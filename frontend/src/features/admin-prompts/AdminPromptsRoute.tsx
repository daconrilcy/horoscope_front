// Surfaces fonctionnelles Admin Prompts: compose les owners feature sous le shell de route.
...
import { useMatchMediaMaxWidth } from "./hooks/useMatchMediaMaxWidth"
...
} from "./adminPromptsPageParts"

export type PromptPageTab = "catalog" | "archive" | "release" | "consumption" | "personas" | "samplePayloads"
...
export function AdminPromptsRoute({ activeTab }: { activeTab: PromptPageTab }) {
                  ...
                  {consumptionQuery.data.data.length === 0 ? (
                    ...
                    <div className="admin-prompts-consumption__cards-mobile" role="list">
                      {consumptionQuery.data.data.map((row) => {
                        ...
                        const periodLabel = formatLocalDateTime(row.period_start_utc)
                        return (
                        ...
                        <table className="admin-prompts-consumption__table">
                          ...
                          <tbody>
                            {consumptionQuery.data.data.map((row) => {
                              ...
                              return (
                                <tr key={rowKey}>
                                  <td>{formatLocalDateTime(row.period_start_utc)}</td>
                                  <td>{axisLabel}</td>
                    ...
                    <div className="admin-prompts-consumption__investigation-head">
                      ...
                      <p className="admin-prompts-consumption__investigation-context" role="status">
                        {tConsumption.selectedRowSummary(
                          formatLocalDateTime(selectedConsumptionRow.period_start_utc),
