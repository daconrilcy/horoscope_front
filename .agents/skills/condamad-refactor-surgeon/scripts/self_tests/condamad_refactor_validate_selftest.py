...
class ValidatePlanSelfTest(unittest.TestCase):
    ...
    def test_missing_domain_fails(self) -> None:
        ...
        errors = self.validate_plan_text(
            valid_plan().replace("- Domain: `backend/app/services`\n", "")
        )
    ...
    def test_missing_validation_commands_fails(self) -> None:
        ...
        plan = valid_plan().replace(
            "pytest backend/app/tests/unit/test_billing.py -q", "echo ok"
        )
        plan = plan.replace(
            "ruff check backend/app/services/billing/service.py", "echo lint"
        )
        plan = plan.replace(
            "condamad_refactor_scan.py backend/app/services/billing --fail-on-hit",
            "echo scan",
        )
        errors = self.validate_plan_text(plan)
...
class EvidenceAndScanSelfTest(unittest.TestCase):
    ...
    def test_evidence_collector_replaces_markers_idempotently(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            collect_evidence.write_snapshot(
                output, collect_evidence.wrap_snapshot(first)
            )
            second = collect_evidence.build_snapshot(root)[0]
            collect_evidence.write_snapshot(
                output, collect_evidence.wrap_snapshot(second)
            )
    ...
    def test_scan_fail_on_hit_returns_non_zero(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            findings = scan_script.scan(
                [target], scan_script.DEFAULT_PATTERNS, Path(tmp)
            )
    ...
    def test_artifact_only_scan_detects_ignored_pycache_path(self) -> None:
        ...
        self.assertTrue(
            any("__pycache__/bad.pyc" in finding[0] for finding in findings)
        )
