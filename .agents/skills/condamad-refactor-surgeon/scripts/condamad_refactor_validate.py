...
def has_required_sections(text: str, sections: list[str]) -> list[str]:
    ...
    return [
        section for section in sections if normalize_heading(section) not in headings
    ]
...
def validate_skill_root(root: Path) -> list[str]:
    ...
    if openai_path.is_file():
        ...
        if "$condamad-refactor-surgeon" not in openai_text:
            errors.append(
                "agents/openai.yaml default prompt must reference $condamad-refactor-surgeon"
            )
...
def validate_primary_domain(section: str) -> list[str]:
    ...
    if len(domains) != 1:
        return [
            f"Primary Domain must declare exactly one '- Domain:' entry, found {len(domains)}"
        ]
...
def validate_scope_boundary(section: str) -> list[str]:
    ...
    if not CONCRETE_SCOPE_RE.search(in_scope) or PLACEHOLDER_RE.search(in_scope):
        errors.append(
            "Scope Boundary must list at least one concrete in-scope path or symbol"
        )
    return errors
...
def validate_plan(path: Path) -> list[str]:
    ...
    text = read_text(path)
    errors = [
        f"Plan missing section: {section}"
        for section in has_required_sections(text, PLAN_SECTIONS)
    ]
    ...
    if not TEST_OR_STATIC_RE.search(validation):
        errors.append(
            "Validation Plan must include at least one test or static command"
        )
    ...
    if not re.search(
        r"git diff --check|git diff --stat|git diff --name-status",
        validation + "\n" + diff_review,
    ):
...
def validate_evidence(path: Path) -> list[str]:
    ...
    if (
        "CONDAMAD:REFACTOR-EVIDENCE:START" not in text
        or "CONDAMAD:REFACTOR-EVIDENCE:END" not in text
    ):
