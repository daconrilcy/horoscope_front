#!/usr/bin/env python3
"""Valide la structure minimale du skill CONDAMAD Story Draft Review.
...
def validate(skill_dir: Path) -> list[str]:
    ...
    if "$condamad-story-draft-review" not in openai_yaml:
        errors.append(
            "agents/openai.yaml default_prompt must mention $condamad-story-draft-review"
        )
    ...
    if "Do not collect git diff baselines" not in step_one:
        errors.append(
            "step-01-target-and-context.md must forbid implementation git diff baselines"
        )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Validate the CONDAMAD story-draft-review skill structure."
    )
    parser.add_argument(
        "skill_dir",
        nargs="?",
        ...
        help="Path to the condamad-story-draft-review skill directory.",
    )
